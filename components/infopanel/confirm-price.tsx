...
const ConfirmPrice: React.FC<IProps> = ({ open, onUpdate, price, close }) => {
  const [update] = useUpdatePriceMutation();
  const selected = useRecoilValue(selectedStation);

  const { register, handleSubmit, reset, watch } = useForm<IUpdateForm>();
  ...
  return (
    <>
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
        ...
        >
          <DialogTitle>{t("prices:confirm:title")}</DialogTitle>
          <DialogContent>
            <DialogContentText>
              {t("prices:confirm:description", {
          ...
          <DialogActions>
            <Button onClick={handleClose}>{t("prices:confirm:cancel")}</Button>
            <Button variant="contained" onClick={handleSubmit(handleUpdate)}>
              {t("prices:confirm:submit")}
            </Button>
          </DialogActions>
        </Box>
      </Dialog>
